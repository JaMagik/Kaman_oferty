...
export async function drawTable(pdfDoc, page, fonts, tableData, startY, title) {
    ...
    for (const [rowIndex, row] of tableData.entries()) {
        ...
        const nameBlockHeight = nameLines.length * tableConfig.contentFontSize * tableConfig.lineHeightMultiplier;
        const descBlockHeight = descLines.length * tableConfig.descriptionFontSize * tableConfig.lineHeightMultiplier;

        const rowHeight = Math.max(nameBlockHeight, descBlockHeight) + tableConfig.padding.top + tableConfig.padding.bottom;
        ...
        const drawCellText = (lines, font, fontSize, cellBounds, align = 'top') => {
            const lineHeight = fontSize * tableConfig.lineHeightMultiplier;
            let textY;

            if (align === 'center') {
                const totalTextHeight = lines.length * lineHeight;
                textY = cellBounds.y + (cellBounds.height / 2) + (totalTextHeight / 2) - (lineHeight - (lineHeight-fontSize)/2)
            } else { // align 'top'
                textY = cellBounds.y + cellBounds.height - tableConfig.padding.top - fontSize;
            }

            lines.forEach(line => {
                const textWidth = font.widthOfTextAtSize(line, fontSize);
                let textX = cellBounds.x + tableConfig.padding.left;
        ...
        drawCellText([String(lp)], regularFont, tableConfig.contentFontSize, { x: columnPositions[0], y: rowY, width: tableConfig.columnWidths[0], height: rowHeight, isCentered: true }, 'center');
        drawCellText(nameLines, regularFont, tableConfig.contentFontSize, { x: columnPositions[1], y: rowY, width: tableConfig.columnWidths[1], height: rowHeight }, 'top');
        drawCellText(descLines, regularFont, tableConfig.descriptionFontSize, { x: columnPositions[2], y: rowY, width: tableConfig.columnWidths[2], height: rowHeight }, 'top');
        drawCellText([String(unit)], regularFont, tableConfig.contentFontSize, { x: columnPositions[3], y: rowY, width: tableConfig.columnWidths[3], height: rowHeight, isCentered: true }, 'center');
        drawCellText([String(quantity)], regularFont, tableConfig.contentFontSize, { x: columnPositions[4], y: rowY, width: tableConfig.columnWidths[4], height: rowHeight, isCentered: true }, 'center');
