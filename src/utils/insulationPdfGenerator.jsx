...
const jobTypeData = {
  sciany: { title: 'OCIEPLENIE ŚCIAN ZEWNĘTRZNYCH', scope: wallScope },
...
export async function generateInsulationOfferPDF(formData) {
  const { userName, price, isNetto, jobs } = formData;
  ...
  try {
    ...
    // Zbieranie unikalnych kart katalogowych
    ...
    for (const job of jobs) {
      ...
      let tableData = JSON.parse(JSON.stringify(details.scope));
      tableData = tableData.map((row, index) => {
        row[0] = String(index + 1);
        // Uzupełnij metraż w odpowiednich wierszach
        if (row[2].includes('m²')) {
            row[4] = job.area;
        }
        return row;
